...
def getPatternsReduced( path, cv, sort):
  ...
  data = norm1(data)
  ps  = np.arange(0,4) # PS
  em1 = np.arange(8,40)# EM1
  em2 = np.arange(72,76) # EM2
  em3 = np.arange(80,84) # EM3
  had1= np.arange(88,90) # HAD1
  had2= np.arange(92,94) # HAD2
  had3= np.arange(96,98) # HAD3
...
parser.add_argument('--reduceRings', action='store_true',
        dest='reduceRings', required = False, default = None,
        help = "Reduce the number of rings by half for each layer (EM1: 64 to 32 rings).")
...
if args.reduceRings:
    f_patterns = getPatternsReduced
else:
    f_patterns = getPatterns

job = BinaryClassificationJob(  PatternGenerator( args.dataFile, f_patterns ),
